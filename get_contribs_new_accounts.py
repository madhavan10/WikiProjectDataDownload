# -*- coding: utf-8 -*-
"""
Created on Mon Feb 14 13:00:03 2022
...
usersDf = pd.read_csv("/home/madhavso/wikipedia_data/user_lists/usersToRetrieve.csv", encoding = "utf-8")

for user in usersDf["usersToRetrieve"]:
    ...
    with open(userCsv, "w", newline = "", encoding = "utf-8") as userCsvF:
        ...
        writer.writerow(("user", "userid", "timestamp", "ns", "pageid", "title", "comment", "revid", "parentid", "size", "sizediff", "minor"))
    ...
    PARAMS = {
            ...
            "list": "usercontribs",
            "ucuser": user,
            "ucprop": "ids|timestamp|title|comment|size|sizediff|flags",
            "ucdir": "newer",
            "uclimit": "max",
            "format": "json"        
        }
    ...
    while True:
        ...
        if "query" in json.keys() and "usercontribs" in json["query"].keys():        
            for contrib in json["query"]["usercontribs"]:
                ...
                contribs.append((user, userid, timestamp, ns, pageid, title, comment, revid, parentid, size, sizediff, minor))
