...
def getRecentUsername(username, driver):
    ...
    def queryRecentUsername(username, driver):
        ...
        usernameURLVersion = username
        if "?" in usernameURLVersion:
            usernameURLVersion = usernameURLVersion.replace("?", "%3F")
        if "\\" in usernameURLVersion:
            usernameURLVersion = usernameURLVersion.replace("\\", "%5C")
        
        driver.get("https://en.wikipedia.org/wiki/User:" + usernameURLVersion)
